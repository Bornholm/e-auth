...
class OpenIDConnectInteractions {
  ...
  _configureApp() {
    ...
    this._provider.on('end_session.success', this._handleSessionEnd.bind(this));
  ...
  _handleSessionEnd(context) {
    // Ensure cookie clearing
    const cookieRegex = /([^=]+)=([^;]+)(;\s)?/g;
    const cookieStr = context.get('cookie');
    let matches;
    while ((matches = cookieRegex.exec(cookieStr))) {
      let cookieName = matches[1];
